...
export function SalesReport() {
  ...
  const paymentRows = completedSales.map((sale) => ({ id: sale.id, date: sale.date, customer: sale.customerName ?? "Customer", method: sale.paymentMethod, invoice: sale.total, paid: sale.paidAmount ?? sale.total, pending: sale.pendingBalance ?? 0 }));
  ...
  return <div className="space-y-6 pb-20 lg:pb-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><Link to="/pos"><Button variant="ghost" size="icon"><ArrowLeft className="h-5 w-5" /></Button></Link><div><h1 className="text-3xl font-bold">Repair POS Reports</h1><p className="mt-1 text-muted-foreground">Daily, monthly, payment and profitability reports for repair invoices</p></div></div><Button onClick={() => setExpenseOpen(true)}><Plus className="mr-2 h-4 w-4" />Add Expense</Button></div>
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Repair Revenue</p><p className="mt-1 text-2xl font-bold">{money(totalSales)}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Pending Payments</p><p className="mt-1 text-2xl font-bold text-warning">{money(pending)}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Expenses</p><p className="mt-1 text-2xl font-bold text-destructive">{money(totalExpenses)}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Net Profit</p><p className="mt-1 text-2xl font-bold">{money(grossProfit - totalExpenses)}</p></CardContent></Card></div>
