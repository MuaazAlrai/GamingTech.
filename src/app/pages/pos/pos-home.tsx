...
import { ChevronDown, History, ShoppingCart } from "lucide-react";
...
export function POSHome() {
  ...
  const todaysSales = sales.filter((sale) => new Date(sale.date).toDateString() === today && (sale.status ?? "completed") === "completed");
  const todaysTotal = todaysSales.reduce((sum, sale) => sum + sale.total, 0);
  const pendingAmount = sales.reduce((sum, sale) => sum + Math.max(0, sale.pendingBalance ?? sale.total - (sale.paidAmount ?? sale.total)), 0);
        ...
        <Card className="border-0 bg-white shadow-sm">
          <CardContent className="p-6">
            <Link to="/pos/sale" className="flex max-w-md items-center gap-4 rounded-xl border bg-white p-5 shadow-sm transition hover:border-primary hover:shadow-md">
              ...
              <div>
                <h2 className="text-xl font-semibold">Sale Invoice</h2>
                <p className="mt-1 text-sm text-muted-foreground">Is par click kar ke Add Sale dashboard open karein.</p>
              </div>
            </Link>
