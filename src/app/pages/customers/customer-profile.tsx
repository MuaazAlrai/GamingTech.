...
export function CustomerProfile() {
  ...
  return <div className="space-y-6 pb-20 lg:pb-6">
    ...
    <Card><CardHeader><CardTitle>Customer Repair Records</CardTitle></CardHeader><CardContent><div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>Invoice Number</TableHead><TableHead>Device</TableHead><TableHead>Status</TableHead><TableHead>Expected Return</TableHead><TableHead>Remaining Days</TableHead></TableRow></TableHeader><TableBody>{customerRepairs.length === 0 ? <TableRow><TableCell colSpan={5} className="py-8 text-center text-muted-foreground">No repair records for this customer.</TableCell></TableRow> : customerRepairs.map((repair) => { const due = getRepairDueState(repair); return <TableRow key={repair.id}><TableCell><Link className="font-medium text-primary hover:underline" to={`/repairs/${repair.id}`}>{repair.ticketNumber || repair.id}</Link></TableCell><TableCell>{repair.device}</TableCell><TableCell>{labelForRepairStatus(repair.status)}</TableCell><TableCell>{repair.estimatedCompletion || "-"}</TableCell><TableCell>{due.label}</TableCell></TableRow>; })}</TableBody></Table></div></CardContent></Card>
